/*============================*/
/* 4th order Runge-Kutta method for ODE (IVP) */
/*    x' = f(t,x)
/*    x(T0)=X0
/*============================*/
#include <stdio.h>
...
/*--- RungeKutta scheme ---*/
double rk4(double h, double t, double x)
{
  double k1,k2,k3,k4;

  k1= f_2(t,x)  ;
  k2= f_2(t+h/2,x+h/2*k1)  ;
  k3= f_2(t+h/2,x+h/2*k2)  ;
  k4= f_2(t+h,x+h*k3)  ;

 return(x+h*(k1+2*k2+2*k3+k4)/6.0);  // x: X_n, t: t_n
}
...
int main(void)
{
 int n;
  ...
  for(n=0; n<N; n++) {
    ...
    x=rk4(h,t,x);  // X_{n+1}=euler(h, t_n, X_n)
